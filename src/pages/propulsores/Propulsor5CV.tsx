...
import { Cog, Zap, Gauge, Droplets, AlertTriangle, Info } from "lucide-react";
...
const Propulsor5CV = () => {
  ...
  const specs = [
    { label: t("propulsors.power"), value: "5 CV", icon: Zap },
    { label: t("propulsors.capacity"), value: t("propulsors.p5cv.capacity"), icon: Gauge },
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      <div className="animate-fade-in">
        <div className="flex items-center gap-3 mb-1">
          ...
          <div>
            <h1 className="text-2xl md:text-3xl font-bold text-foreground tracking-tight">{t("propulsors.p5cv.title")}</h1>
            <p className="text-sm text-muted-foreground">{t("propulsors.p5cv.subtitle")}</p>
          </div>
        </div>
      </div>
      ...
      <Alert className="border-primary/30 bg-primary/5"><Info className="h-4 w-4 text-primary" /><AlertDescription className="text-sm text-muted-foreground">{t("propulsors.p5cv.info")}</AlertDescription></Alert>
      <p className="text-[11px] text-muted-foreground/60 text-center">{t("common.specsFooter")}</p>
    </div>
  );
};
export default Propulsor5CV;
