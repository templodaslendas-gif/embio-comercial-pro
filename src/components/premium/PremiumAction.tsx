...
export function PremiumAction({ icon: Icon, label, description, to, className }: PremiumActionProps) {
  ...
  return (
    <motion.div whileHover={reduceMotion ? undefined : { y: -2 }} transition={{ duration: 0.15, ease: "easeOut" }}>
      <Link
        to={to}
        className={cn(
          "group flex items-center gap-4 rounded-xl border border-border/60 bg-card p-4",
          "hover:border-accent/30 hover:bg-accent/5 transition-colors duration-150",
          className,
        )}
      >
